    ...
    public class TDD_testy {
        ...
        void SUM_TEST()
        {
            ...
            try
            {
                ...
                if (math.Sucet(-5, 20) == 10) Success_test(3);
                else Fail_test(string.Format("Expected: 10 Got: {0}", math.Sucet(-5, 20)), 3);
            }
            catch (InvalidOperationException exception)
            {
                if (exception != null) // ak exception existuje nech j uvypise
                {
        ...
        void DIV_TEST()
        {
            try
            {
                ...
                else Fail_test(string.Format("Expected: 1.5 Got: {0}", math.Delenie(10.5, 10)), 4);

                if (math.Delenie(10.5, 0) == 1.5) Success_test(5);
                else Fail_test(string.Format("Expected: - Got: {0}", math.Delenie(10.5, 0)), 5);
            }
            catch (InvalidOperationException exception)
            {
                Fail_test(string.Format("EXCEPTION = {0}", exception.Message), -1);
